...
import { UploadCloud, X, Lock, Unlock, AlertCircle, Loader2, ArrowRight } from 'lucide-react';
import {
  checkPdfPassword,
  checkDocxPassword,
  checkPptxPassword,
...
export default function ToolModal({ tool, onClose, onLaunchStudio }) {
  ...
  const handleFilesSelected = async (fileList) => {
    ...
    try {
      ...
      // Direct launch to full Studio for Merge PDF and all other tools
        ...
        {tool.id === 'html-to-pdf' && (
          <div className="flex border-b border-slate-100 mb-4 pb-2 space-x-4 text-xs font-semibold">
            <button
              ...
              className={`pb-1 border-b-2 transition cursor-pointer ${htmlMode === 'file' ? 'border-amber-500 text-amber-600' : 'border-transparent text-slate-400'}`}
            >
              Upload .HTML File
            </button>
            <button
              onClick={() => { setHtmlMode('code'); setErrorMsg(''); }}
              className={`pb-1 border-b-2 transition cursor-pointer ${htmlMode === 'code' ? 'border-amber-500 text-amber-600' : 'border-transparent text-slate-400'}`}
            >
              Paste HTML Code
            </button>
          </div>
